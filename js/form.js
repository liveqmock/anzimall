...
(function (factory) {
	...
	$.fn.extend({
		formCheck:function(opts){
			...
			tips = {
				...
				warn:function(dom,value){
					dom.closest('label').siblings('i').attr('class','form-note-warn').html(value);
					opts.failure(the);
				},
				// 错误
				error:function(dom,value){
					dom.closest('label').siblings('i').attr('class','form-note-error').html(value);
					opts.failure(the);
				},
				// 成功
				success:function(dom,value){
					dom.closest('label').siblings('i').attr('class','form-note-success').html(value);
					opts.success(the);
				},
				// 等待
				wait:function(dom,value){
					dom.closest('label').siblings('i').attr('class','form-note-wait').html(value);
				},
				// 取消提示
				notips:function(dom,value){
					dom.closest('label').siblings('i').attr('class','').html(value);
				}
		...
		beautySelect: function(options){
			...
			// 加外层
			this.wrap(function(){return '<div class="fukrm_item fukrm_select"/>';});
			...
			this.on('change',function(){
				...
				var idx = this.selectedIndex;
				this.parentNode.tle.innerHTML = idx<0 ? '' : this.options[idx].text;
				...
				box.cnt.innerHTML = box.slt.innerHTML.replace(/option/gi,'li').replace(/(\svalue)(\=\"?)/gi,'$1$2x');
