...
$(function(){
	...
	$('.banner').marquee({
			time:3,
			distance:550,
			btnGo:{left:$('.banners').find('.toleft'),right:$('.banners').find('.toright')},
			direction:'left'
		});
	

	// var smallban_item = $('.smallban').find('dl').each(function(){
	...
	// 	// 小banner资源轮播
	// 	$this.find('dt').marquee({
	// 		time:3,
	// 		distance:550,
	// 		btnGo:{left:$this.find('.toleft'),right:$this.find('.toright')},
	// 		direction:'left',
	// 		step:function(u){
	// 			dd.hide().eq(Math.abs(u)%len).show();
	// 		}
	// 	});

	// })

	// $('.smallban>.title').on('click','a',function(){

	// 	var $this= $(this);
	// 	var idx = $this.parent().find('a').index($this);

	// 	$this.addClass('active').siblings().removeClass('active');
	// 	smallban_item.eq(idx).addClass('active').siblings().removeClass('active');
	...
	$('.membercard_slides').marquee({
		time:3,
		distance:234,
		btnGo:{left:$('.membercard').find('.toleft'),right:$('.membercard').find('.toright')},
		direction:'left'
	});
