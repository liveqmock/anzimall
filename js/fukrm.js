...
$.fn.extend({
	...
	formCheck:function(opts){
		...
		var rule = {
			...
			// 手机号是否有积分，register_info 页面
			ajax_register_phone2:function(){
				$.ajax({
					type: "POST",
					dataType:'json',
					url: "checkphone.shtml",
					...
					success: function(data) { 
						...
						if (data.status == 201) {
							...
							var res = '<cite><span>用 户 名：</span>'+data.username+'</cite>';
							res += '<cite><span>购机数量：</span>'+data.number+'台</cite>';
							res += '<cite><span>赠送积分：</span>'+data.integral+'</cite>';
							$('.msg').html(res).show();
...
$(function(){
	...
	$('.fukrm').fukrm();
	$(".js-birthday").find('input').jSelectDate({yearBeign: 1960});
