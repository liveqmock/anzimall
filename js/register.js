	...
	// 注册提交
	$('.register_ajax').on('submit',function(){

		var form = $(this)
		var check = [];
		form.children('cite').find('input,textarea,select').filter('[check]').filter(':visible').each(function(){
			var the = $(this);
			if(!the.siblings('i').is('.form-note-success') && !the.formCheck() ){
				check.push(the);
			}
		...
		/*if(!$('.checkbankcard').is('.success')){
			$('.checkbankcard').css('background','#ee2323').next('span').html('未通过校验，请重新输入')
			return false;
		}*/
