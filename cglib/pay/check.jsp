...
<div class="pay">
	...
	<form action="/confirm_pay.shtml" method="post" class="pay_form">
		...
		<div class="pay_inputs">
			...
			<div class="item"><label><em>付款银行：</em><span><img src="/images/${fn:substring(bankno,0,3)}.png"></span></label></div>
			<c:if test="${type eq '1'}">
			<div class="item"><label><em>付款方式：</em>储蓄卡</label></div>
			</c:if>
		...
		<c:if test="${type eq '2'}">
			<div class="item">
				<label><em>有效期：</em><input type="text" value="" name="cardexpirydate"  datatype="datetype" nullmsg="请输入有效期，格式：年份月份" errormsg="格式错误，例：1502" ></label>
				<div class="Validform_checktip">格式：1502</div>
			</div>
