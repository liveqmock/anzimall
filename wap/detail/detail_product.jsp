...
<div class="product_tag">
	<div class="main">
		<a href="/getExchangeGoods_wap.shtml?id=${list_goods.id }" class="active">基本信息</a>
		<a href="/getGoodsDetails_wap.shtml?id=${list_goods.id }">商品详情</a>
	</div>
</div>
	...
	<form action="/toSaveUserExchange_wap.shtml" method="post">
		<section class="product">
			...
			<c:if test="${list_goods.type eq '0'}">
					<c:if test="${empty list_goods.freight }"><h5>运费：<em>0</em>元</h5></c:if>
					<c:if test="${!empty list_goods.freight }"><h5>运费：<em>${list_goods.freight }</em>元</h5></c:if>
			</c:if>
		</section>
